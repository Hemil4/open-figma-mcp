...
import { normalizeNodeId, ScreenRegistryStore } from "./registry.js";
...
function registerTool(name, description, inputSchema, handler) {
    server.registerTool(name, {
        ...
        description,
        inputSchema
    }, async (args) => jsonText(await handler(args)));
}
function registerBridgeTool(spec) {
...
async function prepareBridgeArgs(args, screenTarget) {
    const next = { ...args };
    for (const key of ["nodeId", "parentId", "componentId", "instanceId"]) {
        if (typeof next[key] === "string")
            next[key] = normalizeNodeId(String(next[key]));
    }
    if (Array.isArray(next.nodeIds))
        next.nodeIds = next.nodeIds.map((id) => normalizeNodeId(String(id)));
    ...
    if (screenKey && screenTarget !== "none") {
        ...
        if (!screen)
            throw new Error(`No screen registered for key "${screenKey}". Run figma_create_screen_frames first.`);
        if (screenTarget === "parent" && !next.parentId)
            next.parentId = screen.figmaNodeId;
        if (screenTarget === "node" && !next.nodeId && !next.nodeIds)
            next.nodeId = screen.figmaNodeId;
    }
    return next;
}
const nodeSelector = {
...
for (const spec of bridgeTools)
    registerBridgeTool(spec);
registerTool("figma_export_frames_to_pdf", "Export one or more nodes to PDF. If outputPath is provided, write files on disk.", {
    nodeIds: z.array(z.string()),
    outputPath: z.string().optional()
}, async (args) => {
    const params = await prepareBridgeArgs({ ...args, format: "PDF" }, "none");
    const result = await bridge.request("export_frames_to_pdf", params, 120000);
    if (!args.outputPath)
        return result;
    return writeExports(result, String(args.outputPath), "pdf");
});
registerTool("figma_save_screenshots", "Export screenshots and save them to disk.", {
...
registerTool("figma_create_screen_frames", "Create one Figma frame per sitemap screen and save their node IDs in the local screen registry.", {
    ...
    if (!Array.isArray(data.screens))
        throw new Error("Figma plugin did not return screen frame IDs.");
...
registerTool("figma_get_screen", "Get a registered screen by key, with live Figma node details when the plugin is connected.", { screenKey: z.string() }, async (args) => {
    const screen = await registryStore.findScreen(String(args.screenKey));
    if (!screen)
        throw new Error(`No screen registered for key "${String(args.screenKey)}".`);
    try {
...
async function saveScreenshots(args) {
    ...
    for (let index = 0; index < items.length; index += 1) {
        ...
        const response = await bridge.request("get_screenshot", params, 120000);
...
async function writeExports(result, outputPath, extension) {
    const exports = result.exports || [];
    if (exports.length === 0)
        throw new Error("No exports returned.");
    if (exports.length === 1) {
...
async function writeBase64File(base64, outputPath) {
    const resolved = resolveOutputPath(outputPath);
